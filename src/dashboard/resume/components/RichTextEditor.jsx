...
function RichTextEditor({onRichTextEditorChange,index,defaultValue}) {
 ...
 const GenerateSummeryFromAI = async () => {
  ...
  try {
    ...
    setValue(cleaned);
    // notify parent Experience with a synthetic event
    onRichTextEditorChange({ target: { value: cleaned } });
  } catch (err) {
    console.error('AI error:', err.response?.data || err);
    toast('AI generation failed. Check backend.');
  } finally {
    setLoading(false);
  }
};
  ...
  onChange={e => {
    ...
    onRichTextEditorChange(e); // Experience will update context via updateAll
  }}
>
         <Toolbar>
          <BtnBold />
